    ...
    <div style={{ maxWidth: '1100px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '32px' }}>
      ...
      <div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px' }}>
          <span className="badge badge-crimson">
            {isEn ? 'Hanzi Writing & Stroke Mastery' : 'Kỹ năng viết và nhận diện Hán tự'}
          </span>
          <span className="badge badge-gold">
            {isEn ? 'Core HSK 1' : 'HSK 1 cốt lõi'}
          </span>
        </div>
        <h1 style={{ fontSize: '2.2rem', fontWeight: '800' }}>
          {isEn ? (
            <>
              Hanzi Writing & <span className="gradient-text">Radical Breakdown</span>
            </>
          ) : (
            <>
      ...
      <div className="glass-panel" style={{ padding: '16px 20px' }}>
        <div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginBottom: '10px', fontWeight: '600' }}>
          {isEn
            ? `Select character to practice (${hsk1Data.length} core words):`
      ...
      >
        ...
        <HanziCanvas
          ...
          meaning={isEn ? (currentVocab.meaningEn || currentVocab.meaning) : currentVocab.meaning}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <div className="glass-panel" style={{ padding: '24px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px' }}>
              ...
              <h3 style={{ fontSize: '1.15rem', fontWeight: '700', color: 'var(--text-primary)' }}>
                {isEn ? 'Radical & Character Breakdown' : 'Chiết tự và bộ thủ cấu thành'}
              </h3>
            </div>
            ...
            {matchedRadical ? (
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: '14px', marginBottom: '10px' }}>
                  ...
                  <div>
                    <div style={{ fontSize: '1.1rem', fontWeight: '700', color: 'var(--text-primary)' }}>
                      {isEn
                        ? `Radical ${matchedRadical.radical} (${matchedRadical.pinyin})`
                        : `Bộ ${matchedRadical.hanviet} (${matchedRadical.pinyin})`}
                    </div>
                    <div style={{ fontSize: '0.85rem', color: '#10b981' }}>
                      {isEn
                        ? `Core Meaning: ${matchedRadical.meaningEn || matchedRadical.meaning}`
                ...
                <div style={{ fontSize: '0.9rem', color: 'var(--text-secondary)', lineHeight: '1.5', marginTop: '10px' }}>
                  <strong style={{ color: '#f59e0b' }}>
                    {isEn ? 'Mnemonic Story: ' : 'Câu chuyện ghi nhớ: '}
                  </strong>
                  {isEn ? (matchedRadical.mnemonicEn || matchedRadical.mnemonic) : matchedRadical.mnemonic}
                </div>
              </div>
            ) : (
              <div style={{ padding: '16px', background: 'var(--bg-tertiary)', border: '1px solid var(--border-subtle)', borderRadius: '12px', color: 'var(--text-secondary)', fontSize: '0.9rem' }}>
                {isEn ? (
                  <>
                    This character carries the radical <strong style={{ color: 'var(--accent-crimson)' }}>{currentVocab.radical || 'general'}</strong> with {currentVocab.strokes} strokes.
                  </>
                ) : (
                  <>
            ...
            {/* Mnemonic insight card (Sino-Vietnamese in VI, Semantic composition in EN) */}
            <div
              style={{
                marginTop: '16px',
                padding: '14px',
            ...
            >
              <div style={{ fontSize: '0.88rem', fontWeight: '600', color: 'var(--accent-crimson)', marginBottom: '4px' }}>
                {isEn
                  ? `💡 Character Anatomy: Radical & Root Parts of "${currentVocab.hanzi}"`
                  : `💡 Vũ khí ghi nhớ người Việt: Âm Hán - Việt "${currentVocab.hanviet}"`}
              </div>
              <div style={{ fontSize: '0.82rem', color: 'var(--text-secondary)', lineHeight: '1.5' }}>
                {isEn ? (
                  <>
                    The character <strong style={{ color: 'var(--text-primary)' }}>{currentVocab.hanzi}</strong> ({currentVocab.pinyin}) is composed of standardized stroke components. Recognizing these recurring building blocks makes memorizing vocabulary and writing patterns intuitive.
                  </>
                ) : (
                  <>
            ...
            <div style={{ marginTop: '16px', padding: '14px', background: 'var(--bg-tertiary)', border: '1px solid var(--border-subtle)', borderRadius: '12px' }}>
              <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: '4px' }}>
                {isEn ? 'Practical Example:' : 'Câu ví dụ thực tế:'}
              </div>
          ...
          <div className="glass-panel" style={{ padding: '24px' }}>
            <h3 style={{ fontSize: '1.1rem', fontWeight: '700', marginBottom: '12px', color: 'var(--text-primary)' }}>
              {isEn ? 'Common Essential Radicals' : 'Các bộ thủ cốt lõi phổ biến khác'}
            </h3>
              ...
              {radicalsData.map((rad, idx) => (
                ...
                >
                  ...
                  <div style={{ fontSize: '0.72rem', color: 'var(--text-primary)', fontWeight: '600', marginTop: '2px' }}>
                    {isEn ? rad.pinyin : rad.hanviet}
                  </div>
                  <div style={{ fontSize: '0.68rem', color: 'var(--text-secondary)' }}>
                    {isEn ? (rad.meaningEn || rad.meaning) : rad.meaning}
                  </div>
                </div>
              ))}
            </div>
